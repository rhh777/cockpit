...
这份文档只描述 cockpit 当前对外呈现的能力、明确边界和后续可能扩展。实现细节以 `docs/01-architecture.md` 为准。
...
- 在任一原生 session 后继续提问,每条消息可发给 Claude 或 Codex。
- Follow-up 历史写入 `~/.cockpit/threads/<source>/<id>/followups.jsonl`,不直接写原生 CLI 文件。
- 上下文由原始 session + 既有 follow-up + 当前请求序列化而来。
- Adapter 通过本机已安装并登录的 `claude` / `codex` CLI 子进程运行,不接管账号、token 或网页登录态。
- 生成过程通过 SSE 流式回显,同时落盘;完成、失败、取消都会追加 `turn_status`。
- 默认只读运行:Codex 使用 read-only sandbox,Claude 限制写入/执行类工具。
...
- 一条消息可通过 `@claude @codex` 并行唤醒多个 agent。
...
| CLI 未安装或未登录 | Adapter 启动前检测命令,错误信息提示用户安装并登录官方 CLI |
| CLI 参数或输出格式变化 | Adapter 尽量结构化解析,失败时降级为 assistant_text/meta |
...
| 新 session 来源 | 接入 Cursor / Cline / Aider 等本地 agent 工具的历史格式 |
| 新 agent adapter | 接入本地模型、官方 API Key adapter 或其他 CLI agent |
